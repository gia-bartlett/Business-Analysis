...
Taking into account the type of knowledge stakholders are using:  
**Tacit Knowledge**  
...
Structured sessions - facilitator guides the conversation of the participants, prompts brainstorming and ensures the session fulfills its goal.  
Diverse participants - uncovers gaps, redundancies, needs and other important information.  
Interviews - structured, semi-structured, unstructured (F2F, phone, video, email).  
Collect and anlayse exisiting documentation (ask why certain information is there).

**BAs must ask WHY? - and the answer 'because it's always been done that way' is NOT enough!**  

Shadowing people in department while they perform their duties.  
Large numbers - use surveys and questionnaires with open ended questions.  

**Beware Analysis Paralysis**  
The process can get very lengthy  
Be thorough but constrain ER to 2 phases:  
1. Initial Elicitation during the needs assessment activities  
2. Second assessment after the project has been initiated  
...
#### Models for Analysing Requirements:  
Nouns (data modeling)  
Verbs (process modeling)  
Rules (business rule modeling)  

*'I, as a customer, want to order a camera, to take photo of my family.'*  
to order = verb  
camera = noun  
F2F - with stakeholders to look for verbs (order camera, receive camera, use camera)  
Data modeling defines the nouns (different types of customers, different types of cameras)  

**Pro Tip**  
Use physical representations (sticky notes on a board)  
Use colour to show differences  
Model with stakeholder present to gain their input  
